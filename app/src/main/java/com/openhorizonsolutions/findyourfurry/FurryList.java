...
public class FurryList extends AppCompatActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        Intent furryListIntent = getIntent();
        int listmode = furryListIntent.getIntExtra("listmode", ListMode.LIVE_MODE); // live mode by default

        contestList = (ListView) findViewById(R.id.furryListView);

        if (listmode == ListMode.LIVE_MODE)
        {
            // live mode
        ...
        else
        {
            ...
            adapter = new ArrayAdapter <String>
                    (FurryList.this, android.R.layout.simple_list_item_1,
                            DataStore.withinRangeStringSearch);
            ...
            contestList.setOnItemClickListener(new AdapterView.OnItemClickListener()
            {
                ...
                public void onItemClick(AdapterView arg0, View arg1, int position, long arg3)
                {
                    ...
                    try
                    {
                        DataStore.withinRangeSearch.get(position);
                    }
                    catch (NullPointerException npe)
                    {
                        safetyCheck = true;
                    ...
                    if (!safetyCheck)
                    {
                        final Furry furry = DataStore.withinRangeSearch.get(position);
                        // TODO: add profile picture in the future?
                        // ImageView image = new ImageView(FurryList.this);
                        // image.setImageResource(R.drawable.furrymap);
                        // alertDialog.setView(image);
            ...
            new AlertDialog.Builder(this)
                    ...
                    .setMessage(String.format("%d furries found!~~ UwU", DataStore.withinRangeSearch.size()))
